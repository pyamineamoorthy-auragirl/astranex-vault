...
const CertificateVault = () => {
  ...
  setTimeout(() => {
    const found = studentsData.find(s => s.reg === input.trim());
    if (found) {
      setResult(found);
      
      // TRIGGER CONFETTI FOR WINNERS
      if (found.category === "Winner" && window.confetti) {
        window.confetti({
          particleCount: 150,
          spread: 70,
          origin: { y: 0.6 },
          colors: ['#2dd4bf', '#3b82f6', '#ffffff'] // Astranex Teal & Blue
        });
      }
    } else {
      setError(true);
    ...
    {isLoading ? <Loader2 className="animate-spin" size={20} /> : <Search size={22} strokeWidth={3} />}
          ...
          {result && !isLoading && (
            <div className="animate-in fade-in zoom-in-95 duration-500 bg-gradient-to-br from-white/[0.05] to-transparent border border-white/10 rounded-3xl p-8 text-center">
              ...
              {/* Fake Digital Signature for Premium Feel */}
              ...
              <a 
                href={`/certificates/${result.reg}.pdf`} 
                download
      ...
      <footer className="mt-16 text-[9px] font-black text-slate-600 tracking-[0.5em] uppercase">
        Engineered for SRGEC Inaugural 2026
      </footer>
    </div>
  );
};
